...
export const init = () => reaction(
  () => {
    if (keyService.state.keys?.type === 'mixin') {
      return {
        userId: keyService.state.keys.user.mixin.mixin_userid,
        address: keyService.state.keys.appUser.eth_address,
      };
    }
  },
  async (data) => {
    // TODO: check config if nft was enabled
    if (!data) {
      clearNFT();
      return;
    }
    // const hasNFT = await requestNFT(data.userId)
    // if (hasNFT) {
    //   await getNFTsByAddress(data.address);
    // }
    await getNFTsByAddress(data.address);
  },
);
